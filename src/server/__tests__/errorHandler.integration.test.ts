...
describe("Error Handler Integration Tests", () => {
	...
	describe("401 Unauthorized Errors", () => {
		it("should handle 401 errors in budget tools", {
		...
		it("should handle 401 errors in transaction tools", {
			meta: { tier: "domain", domain: "server" },
		}, async () => {
	...
	describe("403 Forbidden Errors", () => {
		...
		it("should handle 403 errors in payee tools", {
			meta: { tier: "domain", domain: "server" },
		}, async () => {
	...
	describe("404 Not Found Errors", () => {
		it("should handle 404 errors in month tools", {
			meta: { tier: "domain", domain: "server" },
		}, async () => {
	...
	describe("429 Rate Limit Errors", () => {
		it("should handle 429 errors in utility tools", {
			meta: { tier: "domain", domain: "server" },
		}, async () => {
	...
	describe("500 Internal Server Errors", () => {
		it("should handle 500 errors consistently across tools", {
			meta: { tier: "domain", domain: "server" },
		}, async () => {
	...
	describe("Error Response Structure", () => {
		it("should maintain consistent error response structure across all tools", {
			meta: { tier: "domain", domain: "server" },
		}, async () => {
	...
	describe("Sensitive Data Sanitization", () => {
		it("should sanitize sensitive data in error messages across all tools", {
			meta: { tier: "domain", domain: "server" },
		}, async () => {
	...
	describe("Instance consistency", () => {
		it("should produce identical results for two instances with same formatter", {
			meta: { tier: "domain", domain: "server" },
		}, () => {
		...
		it("should produce identical results for createValidationError across instances", {
			meta: { tier: "domain", domain: "server" },
		}, () => {
			const errorHandler1 = createErrorHandler(responseFormatter);
			const errorHandler2 = createErrorHandler(responseFormatter);
	...
	describe("Circular dependency resolution", () => {
		...
		it("should work with different formatter implementations", {
			...
			expect(() =>
				JSON.parse(result.content[0].text.substring(8)),
			).not.toThrow();
		});
	});
	...
	describe("ErrorHandler factory", () => {
		it("should create different instances with different formatters", {
			...
			const formatter1 = {
				format: (v: unknown) => `F1: ${JSON.stringify(v)}`,
			};
			const formatter2 = {
				format: (v: unknown) => `F2: ${JSON.stringify(v)}`,
			};
