...
describe("DeltaCache", () => {
	...
	const createMerger = () =>
		vi
			...
			.mockImplementation((snapshot: TestEntity[], delta: TestEntity[]) => [
				...snapshot,
				...delta,
			]);
	...
	describe("Cache Operations", () => {
		...
		it("should use fresh cache but disable delta when forceFullRefresh is true", async () => {
			cacheManagerSpies.get.mockReturnValue(
				createCacheEntry([{ id: "cached" }], 500),
			);
	...
	describe("Merge Options", () => {
		it("should pass merge options to merger", async () => {
			cacheManagerSpies.get.mockReturnValue(
				createCacheEntry([{ id: "cached" }], 100),
			);
		...
		it("should allow preserveDeleted to keep deleted entities after merge", async () => {
			...
			const merger = vi
				.fn()
				.mockReturnValue([{ id: "cached" }, { id: "delta", deleted: true }]);
	...
	describe("Invalidation", () => {
		it("should delete by prefix when resource type provided", () => {
			...
			expect(cacheManagerSpies.deleteByPrefix).toHaveBeenCalledWith(
		...
		it("should delete by budget when resource type omitted", () => {
			...
			expect(cacheManagerSpies.deleteByBudgetId).toHaveBeenCalledWith(
				"budget-123",
			);
		});
		...
		it("should reset targeted knowledge during forceFullRefresh", () => {
			...
			expect(knowledgeStoreSpies.reset).toHaveBeenCalledWith(
				"transactions:list:budget-123",
			);
		});
		...
		it("should reset entire budget knowledge on forceFullRefresh without resource type", () => {
			...
			expect(knowledgeStoreSpies.resetByBudgetId).toHaveBeenCalledWith(
				"budget-123",
			);
		});
	...
	describe("TTL Validation", () => {
		...
		it("should throw when ttl is not finite for fetchWithoutDelta", async () => {
			const fetchWithoutDelta = (deltaCache as any).fetchWithoutDelta.bind(
				deltaCache,
			);
