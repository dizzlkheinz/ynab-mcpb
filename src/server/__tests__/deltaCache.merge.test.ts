...
describe("mergeFlatEntities", () => {
	...
	it("should keep additional fields intact", () => {
		...
		const result = mergeFlatEntities(snapshot, delta);
		expect(result).toEqual([
			{ id: "1", extra: { nested: false }, label: "v2" },
		]);
	});
});
...
describe("mergeCategories", () => {
	...
	it("should merge new categories within existing group", () => {
		...
		const result = mergeCategories(snapshot, delta);
		expect(result[0].categories.map((cat) => cat.id)).toEqual([
			"cat-1",
			"cat-2",
		]);
	});
	...
	it("should maintain category order after merges", () => {
		...
		const result = mergeCategories(snapshot, delta);
		expect(result[0].categories.map((cat) => cat.name)).toEqual([
			"First",
			"Second",
			"Third",
		]);
	...
	it("should handle multiple groups with multiple category updates", () => {
		const snapshot = [
			...
			buildCategoryGroup({
				id: "group-2",
				categories: [
					buildCategory({ id: "cat-2", category_group_id: "group-2" }),
				],
			}),
		];
		const delta = [
...
describe("mergeTransactions", () => {
	...
	it("should merge new subtransactions into existing transaction", () => {
		const snapshot = [
			buildTransaction({
				id: "txn-1",
				subtransactions: [
					buildSubTransaction({ id: "sub-1", transaction_id: "txn-1" }),
				],
			}),
		];
		const delta = [
			buildTransaction({
				id: "txn-1",
				subtransactions: [
					buildSubTransaction({ id: "sub-2", transaction_id: "txn-1" }),
				],
			}),
		];

		const result = mergeTransactions(snapshot, delta);
		expect(result[0].subtransactions.map((sub) => sub.id)).toEqual([
			"sub-1",
			"sub-2",
		]);
	});
	...
	it("should handle transactions without subtransactions arrays", () => {
		...
		const delta = [
			buildTransaction({ id: "txn-1", memo: "updated", subtransactions: [] }),
		];
...
describe("Merge Edge Cases", () => {
	...
	it("should efficiently handle large snapshots", () => {
		const largeSnapshot = Array.from({ length: 1200 }, (_, index) => ({
			id: `id-${index}`,
		}));
	...
	it("should efficiently handle large delta payloads", () => {
		...
		const largeDelta = Array.from({ length: 1000 }, (_, index) => ({
			id: `delta-${index}`,
		}));
	...
	it("should handle deeply nested category structures", () => {
		...
		const result = mergeCategories(snapshot, delta);
		expect(
			result[0].categories.every((cat) => cat.name?.startsWith("Updated")),
		).toBe(true);
	});
	...
	it("should handle transactions with many subtransactions", () => {
		...
		expect(
			result[0].subtransactions.find((sub) => sub.id === "sub-3")?.amount,
		).toBe(999);
	});
