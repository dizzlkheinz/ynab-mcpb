...
describeIntegration("Transaction Tools Integration", () => {
	...
	it("should successfully list transactions from real API", {
	...
	it("should successfully list transactions with account filter", {
	...
	it("should successfully list transactions with date filter", {
		meta: { tier: "domain", domain: "transactions" },
	}, async (ctx) => {
		await withRateLimitSkip(ctx, async () => {
			const params = {
	...
	it("should get transaction details if transactions exist", {
		meta: { tier: "core", domain: "transactions" },
	}, async (ctx) => {
		await withRateLimitSkip(ctx, async () => {
	...
	it("should handle invalid transaction ID gracefully", {
		meta: { tier: "domain", domain: "transactions" },
	}, async (ctx) => {
		await withRateLimitSkip(ctx, async () => {
			const params = {
	...
	describe("handleCreateTransactions - Integration", () => {
		...
		it("should detect duplicates when reusing import IDs", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
		...
		it("should invalidate caches so new transactions appear in list results", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
		...
		it("should create transactions across multiple accounts within one batch", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
		...
		it("should handle large batches and report response mode", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
		...
		it("should support dry run mode without creating transactions", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
		...
		it("should confirm dry run does not persist data", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
		...
		it("should handle invalid budget IDs gracefully during bulk create", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
				const result = await handleCreateTransactions(ynabAPI, {
		...
		it("should handle invalid account IDs during bulk create", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
				const result = await handleCreateTransactions(ynabAPI, {
	...
	describeIntegration("Bulk Update Transactions Integration", () => {
		...
		it("should successfully update transactions without metadata (using cache/API)", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
		...
		it("should provide before/after preview in dry_run mode", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
				// Create a test transaction
		...
		it("should reject entire batch when any transaction ID is invalid", {
			meta: { tier: "domain", domain: "transactions" },
		}, async (ctx) => {
			await withRateLimitSkip(ctx, async () => {
