...
describeIntegration("Month Tools Integration", () => {
	...
	describe("handleListMonths", () => {
		it("should successfully list months from real API", {
			...
			console.warn(
				`✅ Successfully listed ${parsedContent.months.length} months`,
			);
	...
	describe("handleGetMonth", () => {
		it("should successfully get month details from real API", {
			meta: { tier: "domain", domain: "months" },
		}, async () => {
		...
		it("should handle invalid month format gracefully", {
			...
			console.warn(
				`✅ Correctly handled invalid month: ${parsedContent.error.message}`,
			);
		});
	});
});
