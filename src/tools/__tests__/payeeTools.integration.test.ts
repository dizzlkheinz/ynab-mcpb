...
describeIntegration("Payee Tools Integration", () => {
	...
	describe("handleListPayees", () => {
		it("should successfully list payees from real API", {
			...
			console.warn(
				`✅ Successfully listed ${parsedContent.payees.length} payees`,
			);
			console.warn(`   - ${transferPayees.length} transfer payees`);
			console.warn(
				`   - ${parsedContent.payees.length - transferPayees.length} regular payees`,
			);
		});
	...
	describe("handleGetPayee", () => {
		it("should successfully get payee details from real API", {
			meta: { tier: "domain", domain: "payees" },
		}, async () => {
			...
			if (payee.transfer_account_id) {
				console.warn(
					`   - Transfer payee for account: ${payee.transfer_account_id}`,
				);
			} else {
		...
		it("should handle invalid payee ID gracefully", {
			meta: { tier: "domain", domain: "payees" },
		}, async () => {
			const result = await handleGetPayee(ynabAPI, {
				budget_id: testBudgetId,
			...
			console.warn(
				`✅ Correctly handled invalid payee ID: ${parsedContent.error.message}`,
			);
		});
