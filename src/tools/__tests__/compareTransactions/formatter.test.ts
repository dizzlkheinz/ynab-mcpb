...
describe("formatter", () => {
	...
	describe("findSuggestedPayee", () => {
		...
		test("should find exact payee name match", () => {
			...
			expect(result.suggestion_reason).toBe(
				"Matched payee 'Starbucks' in description.",
			);
		});
		...
		test("should find partial payee name match", () => {
			...
			expect(result.suggestion_reason).toBe(
				"Matched payee 'Amazon' in description.",
			);
		});
		...
		test("should find first matching payee when multiple matches", () => {
			const multiPayees = [
				createPayee("payee1", "Coffee"),
				createPayee("payee2", "Coffee Shop"),
			];
	...
	describe("formatMatches", () => {
		test("should format matches correctly", () => {
			...
			const match = createTransactionMatch(bankTxn, ynabTxn, 95, [
				"Exact match",
				"Perfect score",
			]);
	...
	describe("formatUnmatchedBank", () => {
		...
		test("should handle negative amounts", () => {
			const bankTxns = [
				createBankTransaction("2023-09-15", -123450, "Refund", 1),
			];
	...
	describe("buildComparisonResult", () => {
		test("should build complete comparison result", () => {
			const bankTxns = [
				createBankTransaction("2023-09-15", 123450, "Test 1", 1),
			];
			const ynabTxns = [
				createYNABTransaction("ynab1", "2023-09-15", 123450, "Test Payee"),
			];
			const matches = [createTransactionMatch(bankTxns[0], ynabTxns[0])];
			const unmatchedBank = [
				createBankTransaction("2023-09-16", 67890, "Unmatched", 2),
			];
			const unmatchedYnab = [
				createYNABTransaction("ynab2", "2023-09-17", 99999),
			];
