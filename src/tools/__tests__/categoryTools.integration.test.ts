...
const shouldSkip =
	process.env.SKIP_E2E_TESTS?.toLowerCase() === "true" || !hasToken;
...
describeIntegration("Category Tools Integration", () => {
	...
	describe("handleListCategories", () => {
		it("should successfully list categories from real API", {
			...
			if (parsedContent.categories.length > 0) {
				const availableCategory = parsedContent.categories.find(
					(cat: any) => !cat.hidden,
				);
		...
		it("should handle invalid budget ID gracefully", {
			...
			console.warn(
				`✅ Correctly handled invalid budget ID: ${parsedContent.error.message}`,
			);
		});
	});

	describe("handleGetCategory", () => {
		it("should successfully get category details from real API", {
			meta: { tier: "domain", domain: "categories" },
		}, async () => {
			if (!testCategoryId) {
		...
		it("should handle invalid category ID gracefully", {
			...
			console.warn(
				`✅ Correctly handled invalid category ID: ${parsedContent.error.message}`,
			);
		});
	});

	describe("handleUpdateCategory", () => {
		it("should successfully update category budget from real API", {
			meta: { tier: "domain", domain: "categories" },
		}, async () => {
			if (!testCategoryId) {
				console.warn("⚠️ Skipping test - no test category ID available");
			...
			console.warn(
				`✅ Successfully updated category budget to ${testBudgetedAmount} milliunits`,
			);
			...
			console.warn(
				`✅ Restored original budget amount: ${originalBudgetedAmount} milliunits`,
			);
		});
		...
		it("should handle negative budgeted amounts", {
			meta: { tier: "domain", domain: "categories" },
		}, async () => {
			if (!testCategoryId) {
				console.warn("⚠️ Skipping test - no test category ID available");
