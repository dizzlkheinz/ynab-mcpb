...
describeIntegration("Budget Tools Integration", () => {
	...
	describe("handleListBudgets", () => {
		it("should successfully list budgets from real API", {
			...
			await skipOnRateLimit(async () => {
				...
				console.warn(
					`✅ Successfully listed ${parsedContent.budgets.length} budgets`,
				);
			}, ctx);
		});
	});
	...
	describe("handleGetBudget", () => {
		it("should successfully get budget details from real API", {
			meta: { tier: "core", domain: "budgets" },
		}, async (ctx) => {
			await skipOnRateLimit(async () => {
