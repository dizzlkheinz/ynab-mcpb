...
describe("Date Validation in comparisonOutputs", () => {
	describe("ISODateStringSchema", () => {
		...
		it("should reject invalid months", () => {
			expect(() => ISODateStringSchema.parse("2024-00-15")).toThrow(
				"Invalid calendar date",
			);
			expect(() => ISODateStringSchema.parse("2024-13-15")).toThrow(
				"Invalid calendar date",
			);
		});

		it("should reject invalid days", () => {
			expect(() => ISODateStringSchema.parse("2024-01-00")).toThrow(
				"Invalid calendar date",
			);
			expect(() => ISODateStringSchema.parse("2024-01-32")).toThrow(
				"Invalid calendar date",
			);
			expect(() => ISODateStringSchema.parse("2024-02-30")).toThrow(
				"Invalid calendar date",
			);
			expect(() => ISODateStringSchema.parse("2024-02-31")).toThrow(
				"Invalid calendar date",
			);
			expect(() => ISODateStringSchema.parse("2024-04-31")).toThrow(
				"Invalid calendar date",
			); // April has 30 days
			expect(() => ISODateStringSchema.parse("2024-11-31")).toThrow(
				"Invalid calendar date",
			); // November has 30 days
		});

		it("should reject non-leap year February 29", () => {
			expect(() => ISODateStringSchema.parse("2023-02-29")).toThrow(
				"Invalid calendar date",
			);
			expect(() => ISODateStringSchema.parse("2025-02-29")).toThrow(
				"Invalid calendar date",
			);
		});
	...
	describe("BankTransactionComparisonSchema", () => {
		it("should accept valid bank transaction with valid date", () => {
			...
			expect(() =>
				BankTransactionComparisonSchema.parse(validTransaction),
			).not.toThrow();
		});
		...
		it("should reject bank transaction with invalid date", () => {
			...
			expect(() =>
				BankTransactionComparisonSchema.parse(invalidTransaction),
