...
describe("Transaction Mutation Schema Type Safety", () => {
	describe("TransactionDryRunPreviewSchema", () => {
		it("should accept valid transaction preview", () => {
			...
			expect(() =>
				TransactionDryRunPreviewSchema.parse(validPreview),
			).not.toThrow();
		});
		...
		it("should accept minimal transaction preview with required fields only", () => {
			...
			expect(() =>
				TransactionDryRunPreviewSchema.parse(minimalPreview),
			).not.toThrow();
		});
		...
		it("should reject invalid date format", () => {
			...
			expect(() =>
				TransactionDryRunPreviewSchema.parse(invalidPreview),
			).toThrow();
		});
	...
	describe("SubtransactionPreviewSchema", () => {
		it("should accept valid subtransaction preview", () => {
			...
			expect(() =>
				SubtransactionPreviewSchema.parse(validSubtransaction),
			).not.toThrow();
		});
		...
		it("should accept minimal subtransaction with amount only", () => {
			...
			expect(() =>
				SubtransactionPreviewSchema.parse(minimalSubtransaction),
			).not.toThrow();
		});
		...
		it("should not include server-generated fields", () => {
			...
			const parsed = SubtransactionPreviewSchema.parse(
				subtransactionWithServerFields,
			);
			expect(parsed).not.toHaveProperty("id");
