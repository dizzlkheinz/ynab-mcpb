...
describe("ExportTransactionsOutputSchema", () => {
	it("should validate minimal export mode output", () => {
		const validOutput = {
			...
			full_path:
				"/Users/username/Downloads/ynab_transactions_minimal_2025-11-18.json",
			export_directory: "/Users/username/Downloads",
			export_mode: "minimal",
			minimal_fields: "id, date, amount, payee_name, cleared",
			filename_explanation:
				"Filename format: ynab_{filters}_{count}items_{timestamp}.json",
			preview_count: 10,
			total_count: 150,
		...
		if (result.success) {
			...
			expect(result.data.minimal_fields).toBe(
	...
	it("should validate export output with empty preview", () => {
		const validOutput = {
			...
			full_path:
				"/Users/username/Downloads/ynab_transactions_empty_2025-11-18.json",
			export_directory: "/Users/username/Downloads",
			export_mode: "full",
			minimal_fields: null,
