...
export function calculateMatchScore(
	...
	if (amountDiff === 0) {
		...
		reasons.push(
			`Amount within ${(amountDiffPercent * 100).toFixed(2)}% tolerance`,
		);
	}
	...
	if (
		bankDesc &&
		ynabPayee &&
		(ynabPayee.includes(bankDesc) || bankDesc.includes(ynabPayee))
	) {
		score += 10;
		...
		bankDesc &&
		ynabMemo &&
		(ynabMemo.includes(bankDesc) || bankDesc.includes(ynabMemo))
	) {
		score += 5;
...
export function matchDuplicateAmounts(
	...
	const sortedBank = [...bankTxns].sort(
		(a, b) => a.date.getTime() - b.date.getTime(),
	);
	const sortedYnab = [...ynabTxns].sort(
		(a, b) => a.date.getTime() - b.date.getTime(),
	);
	...
	for (const bankTxn of sortedBank) {
		...
		for (const ynabTxn of sortedYnab) {
			...
			if (enableChronologyBonus) {
				const daysDiff =
					Math.abs(bankTxn.date.getTime() - ynabTxn.date.getTime()) /
					(1000 * 60 * 60 * 24);
			...
			if (
				adjustedScore >= 30 &&
				(!bestMatch || adjustedScore > bestMatch.score)
			) {
				const enhancedReasons = [...reasons];
				if (chronologyBonus > 0) {
					enhancedReasons.push(
						`Chronological order bonus (+${chronologyBonus})`,
					);
				}
				bestMatch = {
					ynab: ynabTxn,
...
export function findMatches(
			...
			bankByAmount
				...
				?.filter(
					(txn): txn is BankTransaction =>
						"raw_amount" in txn &&
						!usedBankIndices.has(bankTransactions.indexOf(txn)),
				) || [];
		const ynabDuplicates =
			ynabByAmount
				.get(amount)
				?.filter(
					(txn): txn is YNABTransaction =>
						"id" in txn && !usedYnabIds.has(txn.id),
				) || [];
	...
	for (let i = 0; i < bankTransactions.length; i++) {
		...
		if (
			!bankTxn ||
			usedBankIndices.has(i) ||
			duplicateAmounts.has(bankTxn.amount)
		)
			continue;
		...
		for (const ynabTxn of ynabTransactions) {
			if (usedYnabIds.has(ynabTxn.id) || duplicateAmounts.has(ynabTxn.amount))
				continue;
	...
	const unmatched_bank = bankTransactions.filter(
		(_, i) => !usedBankIndices.has(i),
	);
	const unmatched_ynab = ynabTransactions.filter(
		(txn) => !usedYnabIds.has(txn.id),
	);
