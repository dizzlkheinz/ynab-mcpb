...
async function verifySimpleEndpoint({
	name,
	signature,
	fetchFull,
	fetchDelta,
...
async function main() {
	...
	await verifySimpleEndpoint({
		...
		fetchDelta: (knowledge) =>
			ynabAPI.budgets.getBudgetById(budgetId, knowledge),
	});
	...
	await verifySimpleEndpoint({
		...
		fetchDelta: (knowledge) =>
			ynabAPI.accounts.getAccounts(budgetId, knowledge),
	});
	...
	await verifySimpleEndpoint({
		...
		fetchDelta: (knowledge) =>
			ynabAPI.categories.getCategories(budgetId, knowledge),
	});
	...
	await verifySimpleEndpoint({
		...
		fetchDelta: (knowledge) =>
			ynabAPI.months.getBudgetMonths(budgetId, knowledge),
	});
	...
	await verifySimpleEndpoint({
		...
		fetchDelta: (knowledge) =>
			ynabAPI.scheduledTransactions.getScheduledTransactions(
				budgetId,
				knowledge,
			),
	});
