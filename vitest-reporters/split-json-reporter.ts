...
export default class SplitJsonReporter implements Reporter {
	...
	async onFinished(files?: File[]) {
		...
		for (const file of files) {
			...
			summary.numPassedTests += allTasks.filter(
		...
		if (failedTests.length > 0) {
			await writeFile(
				...
				JSON.stringify(
					{ numFailedTests: failedTests.length, failures: failedTests },
					null,
					2,
				),
				"utf-8",
		...
		const pointerContent = {
			...
			files: {
				"summary.json":
					"High-level overview with totals and project breakdowns",
				...
				"failed-tests.json":
					"Detailed failure information (only present if tests failed)",
	...
	private getAllTasks(file: File): TaskNode[] {
		const tasks: TaskNode[] = [];

		const collectTasks = (suite: TaskNode) => {
			if (suite.tasks) {
